...
async function flixShowMovies() {
  ...
  flixListEl.innerHTML = flixPriorSearch.map((movie) => flexHTML(movie)).join('')
}
...
async function flixInfo(query = currentQuery, filter) {
  ...
  try {
    ...
    // Remove the 'flix__loading' class once movie are loaded to stop the loading indication.
...
// A function to handle filtering movies years based on user input from an event (e.g., a dropdown change).
function filterYears(event) {
  const filter = event.target.value;  // Get the selected filter value

  // Get the selected filter value and pass it to the flixInfo function.
